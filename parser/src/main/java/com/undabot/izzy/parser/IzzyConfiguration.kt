...
class IzzyConfiguration(resourceTypes: Array<Class<out IzzyResource>> = arrayOf()) {
    ...
    private val registeredTypes: HashMap<String, Class<out IzzyResource>> =
        registeredTypesFrom(resourceTypes)
    ...
    private fun registeredTypesFrom(
        resourceTypes: Array<Class<out IzzyResource>>
    ): HashMap<String, Class<out IzzyResource>> {
        ...
        resourceTypes.iterator().forEach { resourceClass ->
            if (resourceClass.isAnnotationPresent(Type::class.java)) {
                ...
                throw IllegalArgumentException(
                    "Provided $resourceClass is missing " +
                        "com.undabot.izzy.annotations.Type annotation!"
                )
            }
        }
